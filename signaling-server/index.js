...
const transporter = nodemailer.createTransport({
  ...
  auth: {
    ...
    // This value is now pulled from Render's Environment Variables dashboard
...
// Dedicated endpoint for call invites (used by mobile clients to wake killed apps).
app.post("/api/call-invite", async (req, res) => {
  ...
  if (admin.apps.length === 0) {
    return res
      .status(500)
      .json({ ok: false, error: "Server missing Firebase configuration." });
  }
  ...
  const message = {
    ...
    apns: {
      headers: { "apns-priority": "10", "apns-expiration": "30" },
      payload: {
        aps: {
          "content-available": 1,
          sound: "default",
...
io.on("connection", (socket) => {
  ...
  socket.on("webrtc_ice_candidate_user", ({ uid, callId, fromUid, candidate }) => {
    emitToUid(uid, "webrtc_ice_candidate", {
